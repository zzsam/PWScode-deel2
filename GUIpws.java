...
public class GUIpws implements ActionListener{			// We gebruiken dit omdat we dit toen het makkelijkst vonden
	...
	private void initialize() {
		...
		frmEquationSolver.setBounds(100, 100, 715, 482);							//Het frame bepaalde grootes en andere eigenschappen geven
...
//------------------------------------------------------------------------------------- METHODES ---------------------------------------------------------------------------------------
	...
	public static void omschrijven(){      // Het herkennen van verschillen groepen met zijn eigen tekens
		...
		while( Continue == true && i<saveText.length()-1){
			...
			while(saveText.charAt(i) == '+' || saveText.charAt(i) == '-' || saveText.charAt(i) == '*' || saveText.charAt(i) == '/' || saveText.charAt(i) == '='){
				i++;
	...
	public static void doubleOnttrekken(String[] strings){// hier worden de groepen herkend
		 ...
		 double[] B = new double[AantalGroepen];
...
//patrooon dat te maken heeft met de vergelijking die je invoert. //d betekent dat het een digit kan zijn. Het punt betekent dat er een decimaal is. Sin,cos,tan en log kunnen er ook in zitten.
		Pattern p = Pattern.compile("(\\d*.\\d*)\\s?(sin|cos|tan|log)?\\s?(.*)");		// De dingen waar de matcher naar moet zoeken
		  ...
		  for(int i=0 ;i<strings.length;i++){
		     ...
		     if( negatief == true){
		      ...
		      while (index <= extractedRemainder.length()-1){              //Dit deel gaat de remainder langs
				...
				if(start <= extractedRemainder.length()-1 && (extractedRemainder.charAt(start) == '+' ||extractedRemainder.charAt(start) == '-'||extractedRemainder.charAt(start) == '*'||extractedRemainder.charAt(start) == '/'||extractedRemainder.charAt(start) == '=') ){
		  ...
		  if (savei<AantalGroepen-2 && savei < i && equal ==true){
			  A[i] = A[i] * -1;					// als er meerdere groepen na een = staan worden die negatief gemaakt(ze worden naar de andere kant gehaald
		  }
		  else if ( savei < i && equal ==true){
			  
			  antwoord = A[i];						// als er maar 1 groep is na een = dan wordt dat het antwoord
		  }
		
		  if(wanneerNegatief == i){
			  A[i] = A[i] * -1;				// als de groep een negatief teken voor zich had wordt het nu negatief gemaakt
		  }
		  }

		  // rekenen wordt geopend
		  Rekenen.toewijzen(Rekenen2.timesA(A), Rekenen2.timesB(B),	Rekenen2.timesLog( log), Rekenen2.timesSin( sin), Rekenen2.timesCos( cos), Rekenen2.timesTan( tan), Rekenen2.timesAtotX( aTotX), Rekenen2.timesX(xofniet),A,B, aTotX, antwoord);
	}
//------------------------------------------------------------------------------------- parseDouble---------------------------------------------------------------------------------------
